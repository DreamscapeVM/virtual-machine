...
#include <secure/cipher_base64.h>
...
// 파일을 읽어서 vector<uint8_t>로 반환하는 함수
vector<uint8_t> read(const string& filePath) {
    ...
    // vector<uint8_t>를 파일 크기에 맞게 초기화
    vector<uint8_t> buffer(fileSize);

    // 파일 내용을 vector<uint8_t>로 읽기
...
void save(const vector<uint8_t>& data, const string& file_path) {
...
inline auto convert(const string& key) {
    vector<uint8_t> output(key.size());
    for (int i = 0; i < key.size(); i++) {
        output[i] = static_cast<uint8_t>(key[i]);
    }
    return output;
}

template<int size>
inline std::array<uint8_t, size> convert(const string& key) {
