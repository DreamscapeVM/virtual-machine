...
vector<uint8_t> cipher_base64::encypt(const vector<uint8_t> &input) const {
    ...
    for (size_t i = 0; i < input_size; i += 3) {
        ...
        for (int j = 0; j < 4; ++j) {
            if (i + j * 6 <= input_size * 8) {
                result.push_back((uint8_t)base64_chars[(combined >> (18 - j * 6)) & 0x3F]);
            } else {
...
vector<uint8_t> cipher_base64::dencypt(const vector<uint8_t> &input) const {
    std::vector<uint8_t> result;
    ...
    for (size_t i = 0; i < input_size; i += 4) {
        ...
        for (int j = 0; j < 3; ++j) {
            if (i + j < input_size - 1) {
                result.push_back((uint8_t)((combined >> (16 - j * 8)) & 0xFF));
            }
        }
    }
