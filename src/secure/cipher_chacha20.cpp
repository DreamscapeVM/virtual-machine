...
#include <secure/cipher_chacha20.h>
...
void unpack4(const uint32_t& src, uint8_t *dst) {
    dst[0] = static_cast<uint8_t>((src >> 0*8) & 0xff);
    dst[1] = static_cast<uint8_t>((src >> 1*8) & 0xff);
    dst[2] = static_cast<uint8_t>((src >> 2*8) & 0xff);
    dst[3] = static_cast<uint8_t>((src >> 3*8) & 0xff);
}
...
void next(array<uint8_t, 64>& keystream8, array<uint32_t, 16>& state) {
    uint32_t temp32[16];
...
array<uint32_t, 16> init(const array<uint8_t, 32>& key, const array<uint8_t, 8>& nonce) {
...
cipher_chacha20::cipher_chacha20(array<uint8_t, 32> key, array<uint8_t, 8> nonce)  : key(key), nonce(nonce) {}

vector<uint8_t> cipher_chacha20::encypt(const vector<uint8_t> &input) const {
    array<uint8_t, 64> keystream8;
...
vector<uint8_t> cipher_chacha20::dencypt(const vector<uint8_t> &input) const {
    return encypt(input);
}
